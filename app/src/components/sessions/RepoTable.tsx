	...
	return (
		<div className={styles.wrap}>
			<div className={styles.headRow}>
				<span className={styles.headCell}>{t('이름')}</span>
				<span className={styles.headCell}>{t('경로')}</span>
				<span className={styles.headCell}>{t('기본 브랜치')}</span>
				<span className={styles.headCell}>{t('설명 (자동배정 판단 근거)')}</span>
			...
			{repos.length === 0 && <div className={styles.empty}>{t('등록된 레포 없음 — 이 프로젝트가 단일 레포면 안 채워도 됩니다.')}</div>}
			<div className={styles.footer}>
				...
				<div style={{ flex: 1 }} />
				{repos.length === 1 && <span className={`m ${styles.hintText}`}>{t('1개뿐이면 자동배정 안 함 — 지금처럼 단일 레포로 동작')}</span>}
				<span className={`m ${styles.countText}`}>{tp('{n}개 레포', { n: repos.length })}</span>
			</div>
		</div>
	)
}

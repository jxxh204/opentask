	...
	return (
		<Modal open={open} onClose={onClose} width={480}>
			<div style={{ padding: '20px 22px' }}>
				<div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)', marginBottom: 4 }}>폴더 선택</div>
				...
				<div className="scroll-y" style={{ height: 320, border: '1px solid var(--line2)', borderRadius: 10, background: 'var(--bg)' }}>
					{loading && <div style={{ padding: 16, fontSize: 12, color: 'var(--t3)' }}>불러오는 중…</div>}
					...
					{!loading && !error && (
						<>
							...
							{entries.length === 0 && parent == null && <div style={{ padding: 16, fontSize: 12, color: 'var(--t3)' }}>하위 폴더 없음</div>}
							{entries.map((e) => (
								<div
									key={e.path}
									onClick={() => load(e.path)}

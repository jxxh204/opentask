...
server.registerTool(
	'report_task_verify',
	{
		title: '태스크 검증 자료 보고',
		description:
			'이 태스크를 사람이 눈으로 확인할 방법을 현황판(StatusBoard)에 보고한다 — 로컬서버 URL, 스크린샷 경로, 확인용 명령어나 로그 위치 등(웹 화면이 아니어도 된다). 확인할 방법이 여러 개면 각각 따로 여러 번 불러라 — 부를 때마다 목록에 쌓인다(완전히 같은 내용이면 시각만 갱신).',
		inputSchema: {
			taskId: z.string(),

...
const REGISTRY = {
	...
	'task.estimateDuration.judge': {
		...
		template: [
			...
			'추정 대상은 "Claude Code가 구현을 마치고, 개발자가 직접 실행/테스트해서 완료 확인까지" 걸리는 영업일(주말 제외)이다 — 순수 타이핑 속도가 아니라 (a)Claude 결과물이 한 번에 맞게 나올지 vs 리뷰 후 재요청 라운드가 몇 번 필요할지, (b)개발자가 실제로 눌러보고 검증해야 할 케이스·화면·기기가 몇 개인지가 진짜 병목이다.',
			...
			'아래 네 항목을 각각 독립적으로 추정해라 — 해당 없으면 0일도 가능하다. 대부분의 단순~중간 난이도 작업은 Claude가 당일에 구현하므로 항목당 0~1일이 기본값이고, 2일 이상은 "왜 한 번에 안 끝나는지"(재작업 라운드, 넓은 수동 테스트 범위, 설계 불확실성)가 분명할 때만 줘라:',
			'1) 화면 퍼블리싱: 새로 만들거나 손대야 할 화면/컴포넌트 개수와, 개발자가 직접 눈으로 확인해야 할 반응형·인터랙션 케이스 수',
			'2) 로직 개발: 비즈니스 로직·상태관리·API 연동·네이티브 브릿지의 복잡도와, 개발자가 직접 시나리오별로 실행해봐야 할 테스트 케이스 수',
			'3) 설계: 기존 구조에 자연스럽게 들어가는지, 새 추상화·스키마 변경이 필요해 Claude 혼자 판단하기 애매해 사람 확인 라운드가 늘어나는지',
			'4) 영향범위: 몇 개 파일/모듈(웹·모바일 양쪽 다 손대야 하는지 포함)에 걸치는지, 다른 기능에도 파급되는 공용 코드를 건드려 회귀 테스트 범위가 넓어지는지',
			...
			'{"breakdown":[{"item":"화면","days":정수,"note":"짧은 근거"},{"item":"로직","days":정수,"note":"짧은 근거"},{"item":"설계","days":정수,"note":"짧은 근거"},{"item":"영향범위","days":정수,"note":"짧은 근거"}],"detail":"3~6문장 상세 리포트","plan":["1. ...","2. ...","..."],"changes":[{"path":"파일 경로","isNew":false,"summary":"무엇이 바뀌는지 한 줄","toBe":"가벼운 코드 스케치(최대 12줄)"}],"betterDesc":"보강된 설명 3~6문장"}',
			'',
			'제목: {title}',
			'설명: {desc}',
		].join('\n'),

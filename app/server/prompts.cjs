...
const REGISTRY = {
	...
	'link.brief.notion': {
		group: '개발 브리핑',
		label: '링크 브리핑 — 노션',
		desc: '태스크/서브태스크 설명에 붙은 노션 링크를 헤드리스 claude+Notion MCP로 읽어 "이것만 보면 개발 가능한" 핵심 정책 요약을 뽑는다(§ db.cjs v28 link_briefs, linkBrief.cjs). 자동 생성(링크 감지 시 백그라운드).',
		vars: ['url'],
		template: [
	...
	'link.brief.figma': {
		group: '개발 브리핑',
		label: '링크 브리핑 — 피그마',
		desc: '태스크/서브태스크 설명에 붙은 피그마 링크를 헤드리스 claude+Figma MCP로 읽어 핵심 정책 요약을 뽑는다(완성 이미지 자체는 별도로 figma.cjs의 로컬 Dev Mode MCP 스크린샷 경로로 받는다 — 이 프롬프트는 텍스트만). 자동 생성.',
		vars: ['url'],
		template: [
	...
	'code.brief.pre': {
		group: '개발 브리핑',
		label: '코드 브리핑 — 착수 전 참고',
		desc: '서브태스크 착수 시(§ orchestrator.cjs launchSubtask) 저장소에서 관련된 기존 결정 로직을 grep/read로 찾아 file:line + 조건 + API 응답 설명으로 정리(예: 버튼타입 분기 코드). 관련 스토리북 스토리도 감지해 storybookHint로 넘긴다(정확한 딥링크는 codeBrief.cjs가 파일을 직접 파싱해 결정론적으로 만든다 — 모델은 파일 경로/스토리명만).',
		vars: ['taskName', 'subtaskName', 'desc', 'hints'],
		template: [
			'너는 코드베이스 조사관이다. 아래 서브태스크에 착수하기 전에, 이 저장소(현재 작업 디렉토리)에서 관련 있는 기존 결정 로직을 찾아 개발자가 착수 전에 참고할 자료로 정리해라.',
			'⚠️ 이 작업은 오직 읽기 전용 조사·요약이다. 이 워크트리에는 지금 실제로 다른 에이전트가 작업 중일 수 있다 — 어떤 파일도 만들거나 고치지 마라, git 상태를 바꾸는 명령(commit·checkout·branch·reset·add·stash·push 등)을 절대 실행하지 마라. grep/read/ls처럼 읽기만 하는 명령만 써라.',
			'목표는 "이 화면/기능이 실제로 어떤 조건에서 어떻게 동작하는지"를 실제 코드로 보여주는 것이다 — 예시: 버튼 종류가 조건문(buttonType===\'WL\' && urlType===\'B2C_DEFAULT\')에 따라 갈리고 API가 그 값을 어떻게 내려주는지.',
			'도구 호출은 grep/read/bash 합쳐 최대 8회까지만. 실제로 열어본 파일의 file:line과 그 조건/코드를 references에 최대 5개까지 적어라 — 지어내지 마라, 못 찾으면 빈 배열로 둬라.',
			'이 태스크와 관련된 기존 API 엔드포인트가 있으면 endpoints에 method+path+note(그 엔드포인트가 지금 뭘 하는지)로 최대 5개.',
			'관련된 Storybook 스토리 파일(*.stories.tsx 또는 *.stories.ts)을 찾으면 storybookHint에 path(저장소 루트 기준 상대경로)와 story(그 안에서 export된 스토리 이름, 예: "Primary")를 적어라(못 찾으면 null) — 정확한 URL은 네가 만들지 마라, 서버가 파일을 직접 읽어 만든다.',
			'summary는 조사 결과를 2~4문장으로.',
			'설명·코드블록 없이 아래 JSON 객체 "하나만" 출력해:',
			'{"summary":"2~4문장","endpoints":[{"method":"GET","path":"/api/...","note":"..."}],"references":[{"path":"저장소 루트 기준 상대경로","lines":"12-40","condition":"실제 조건/코드 한 줄","explanation":"..."}],"storybookHint":{"path":"...","story":"..."} 또는 null}',
			'',
			'태스크: {taskName}',
			'서브태스크: {subtaskName}',
			'설명: {desc}',
	...
	'code.brief.post': {
		group: '개발 브리핑',
		label: '코드 브리핑 — 완료 후 변경점',
		desc: '서브태스크 완료 시(§ orchestrator.cjs advanceSubtaskWork) 실제 git diff를 근거로 엔드포인트별 변경점 + 바뀐 결정 로직을 file:line으로 정리. pre 단계와 같은 JSON 스키마.',
		vars: ['taskName', 'subtaskName', 'diff'],
		template: [
			'너는 코드 리뷰어다. 아래는 이 서브태스크가 실제로 바꾼 git diff다.',
			'⚠️ 이 작업은 오직 읽기 전용 분석·요약이다. 이 워크트리는 다음 서브태스크가 이어받아 작업할 수 있다 — 어떤 파일도 만들거나 고치지 마라, git 상태를 바꾸는 명령(commit·checkout·branch·reset·add·stash·push 등)을 절대 실행하지 마라. 필요하면 grep/read처럼 읽기만 하는 명령만 써서 diff에 나온 file:line 주변 맥락을 추가로 확인해라.',
			'이 변경으로 어떤 API 엔드포인트가 추가·수정·삭제됐는지, 그 결정 로직이 실제로 어떻게 바뀌었는지 diff에 있는 내용만 근거로 file:line 기준 정리해라 — 지어내지 마라.',
			'endpoints에는 method+path+note(무엇이 바뀌었는지 — 신규/응답 필드 변경/조건 변경 등)를 최대 8개.',
			'references에는 diff에서 실제로 바뀐 결정 로직(조건문 등)의 file:line과 그 의미를 최대 6개(condition에 바뀐 조건/코드, explanation에 그게 왜 중요한지).',
			'storybookHint는 이번 변경으로 새로 생겼거나 바뀐 스토리 파일이 diff에 보이면 path+story, 없으면 null.',
			'summary는 전체 변경을 2~4문장으로.',
			'설명·코드블록 없이 아래 JSON 객체 "하나만" 출력해:',
			'{"summary":"2~4문장","endpoints":[{"method":"GET","path":"/api/...","note":"..."}],"references":[{"path":"저장소 루트 기준 상대경로","lines":"12-40","condition":"...","explanation":"..."}],"storybookHint":{"path":"...","story":"..."} 또는 null}',
			'',
			'태스크: {taskName}',
			'서브태스크: {subtaskName}',
			'[git diff]',
